...
def test_tiny_but_perfect_curve_is_not_scoring_quality():
    ...
    assert result.effect_size_pass is False
    assert not result.quality_pass
    assert "effect threshold" in result.message
...
def test_pipeline_reports_quality_diagnostics():
    ...
    for column in ["dose_response_boundary_flags", "dose_response_high_uncertainty", "dose_response_mean_monotonicity"]:
